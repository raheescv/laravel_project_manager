        ...
        <BarcodeScanner
          ...
          onUpdate={(err, result) => {
            if (result && result.text) {
              const clean = result.text.replace(/\s+/g, '');
              console.log('🔎 Scanned barcode:', clean);
              if (clean.length >= 4 && clean.length <= 30) {
                applyScannedCode(clean); // fetch product and highlight
                closeScanner();           // close modal after scan
              }
            } else if (err) {
              console.error('Scanner error:', err);
            }
          }}
          onError={(err) => console.error('Scanner init error:', err)}
        />
      </div>
