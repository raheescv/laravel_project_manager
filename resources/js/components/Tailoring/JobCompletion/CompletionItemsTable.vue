    ...
    <div class="flex flex-col gap-4 completion-items-table">
        ...
        <div
            ...
            <div class="p-4 sm:p-5 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                <div class="flex items-center gap-3 sm:gap-4 min-w-0">
                    <div
                        class="shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-blue-100 flex items-center justify-center text-blue-700 shadow-sm">
                        <i class="fa fa-tasks text-lg sm:text-xl"></i>
                    </div>
                    <div class="min-w-0">
                        <h3
                            class="flex items-center gap-2 text-sm sm:text-base font-bold text-slate-800 leading-tight truncate">
                            <i class="fa fa-clipboard text-blue-600"></i> Job Completion Items
                        </h3>
                        <p class="flex items-center gap-1.5 text-slate-600 text-xs font-medium"><i
                                class="fa fa-list text-slate-500"></i> {{ items.length }} items total</p>
                    </div>
                </div>
                <div
                    class="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 border-slate-200 pt-3 sm:pt-0">
                    <span class="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><i
        ...
        <div v-if="items.length === 0"
            ...
            <div class="flex flex-col items-center gap-3">
                ...
                <h5 class="flex items-center justify-center gap-2 text-sm font-bold text-slate-700"><i
                        class="fa fa-inbox text-slate-500"></i> No items found</h5>
                <p class="flex items-center justify-center gap-2 text-sm text-slate-600"><i
                        class="fa fa-info-circle text-slate-400"></i> Search for an order to see completion items</p>
            </div>
        </div>
        ...
        <template v-else>
                    ...
                    <div class="p-3 border-b border-slate-200 flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                <span
                                    class="text-xs font-bold text-blue-700 bg-blue-100 px-2 py-0.5 rounded border border-blue-200">#{{
                                        item.item_no }}</span>
                                <span class="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><i
                                        class="fa fa-tag text-slate-500"></i> {{ item.category?.name }}</span>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-xs text-slate-600">
                                <span class="inline-flex items-center gap-1"><i class="fa fa-cubes text-slate-500"></i>
                                    Qty: <strong class="text-slate-800">{{ Number(item.quantity) }}</strong></span>
                                <span class="inline-flex items-center gap-1"><i class="fa fa-tag text-slate-500"></i>
                                    Price: <strong class="text-blue-700">{{ formatCurrency(item.total)
                                        }}</strong></span>
                            </div>
                    ...
                    <div class="p-3 space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div class="rounded-lg p-2.5 border border-slate-200 bg-slate-50/50">
                                <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-1"><i
                                        class="fa fa-user text-blue-600"></i> Assign Tailor</label>
                                <SearchableSelect :modelValue="item.tailor_id"
                                    @update:modelValue="item.tailor_id = $event" :options="tailors"
                                    placeholder="Select tailor..." input-class="assign-tailor-dropdown" />
                            </div>
                            <div class="rounded-lg p-2.5 border border-slate-200 bg-slate-50/50">
                                <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-1"><i
                                        class="fa fa-money text-amber-600"></i> Commission/Item</label>
                                ...
                                <div class="flex justify-between mt-1 text-xs">
                                    <span class="flex items-center gap-1 text-slate-600"><i
                                            class="fa fa-calculator text-slate-500"></i> Total</span>
                        ...
                        <div class="rounded-lg p-2.5 border border-slate-200 bg-slate-50/50">
                            <div class="flex justify-between items-center mb-2">
                                <span class="flex items-center gap-1.5 text-xs font-semibold text-slate-700"><i
                                        class="fa fa-building text-slate-600"></i> Material</span>
                                <span class="flex items-center gap-1 text-xs font-bold text-slate-800"><i
                                        class="fa fa-cubes text-slate-500"></i> Stock: {{ (item.product?.stock_quantity
                                            || 0).toFixed(3) }}</span>
                            </div>
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label
                                        class="flex items-center gap-1 text-[10px] font-semibold text-slate-600 mb-0.5"><i
                                            class="fa fa-arrow-down text-slate-500"></i> Used</label>
                                ...
                                <div>
                                    <label
                                        class="flex items-center gap-1 text-[10px] font-semibold text-slate-600 mb-0.5"><i
                                            class="fa fa-trash text-slate-500"></i> Waste</label>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-1"><i
                                        class="fa fa-calendar text-blue-600"></i> Completion Date</label>
                            ...
                            <div>
                                <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-1"><i
                                        class="fa fa-cubes text-emerald-600"></i> Completed Qty</label>
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-200">
                            <div class="flex items-center gap-1">
                                <span class="flex items-center gap-1.5 text-xs font-semibold text-slate-600 mr-1"><i
                                        class="fa fa-star-half-o text-amber-500"></i> Rating</span>
            ...
            <div class="hidden md:block rounded-2xl overflow-hidden shadow-md border border-slate-200/80 bg-white">
                <p
                    class="px-4 py-2 text-xs text-slate-500 bg-slate-100 border-b border-slate-200 lg:hidden flex items-center justify-center gap-2">
                    <i class="fa fa-arrows-h"></i> Scroll horizontally to see all columns.
                </p>
                    ...
                    <table class="w-full text-left border-collapse min-w-[880px]">
                        <thead>
                            <tr class="bg-slate-100 border-b-2 border-slate-200">
                                ...
                                <th
                                    class="px-3 py-3 lg:px-5 lg:py-4 text-xs font-bold text-slate-700 uppercase tracking-wide">
                                    <span class="inline-flex items-center gap-1.5"><i
                                            class="fa fa-list text-blue-600"></i> Item Details</span>
                                </th>
                                <th
                                    class="px-3 py-3 lg:px-5 lg:py-4 text-xs font-bold text-slate-700 uppercase tracking-wide">
                                    <span class="inline-flex items-center gap-1.5"><i
                                            class="fa fa-user text-blue-600"></i> Tailor</span>
                                </th>
                                <th
                                    class="px-3 py-3 lg:px-5 lg:py-4 text-xs font-bold text-slate-700 uppercase tracking-wide">
                                    <span class="inline-flex items-center gap-1.5"><i
                                            class="fa fa-building text-blue-600"></i> Material</span>
                                </th>
                                <th
                                    class="px-3 py-3 lg:px-5 lg:py-4 text-xs font-bold text-slate-700 uppercase tracking-wide">
                                    <span class="inline-flex items-center gap-1.5"><i
                                            class="fa fa-calendar text-blue-600"></i> Date / Qty</span>
                                </th>
                                <th
                                    class="px-3 py-3 lg:px-5 lg:py-4 text-xs font-bold text-slate-700 uppercase tracking-wide text-center">
                                    <span class="inline-flex items-center gap-1.5 justify-center"><i
                                            class="fa fa-cogs text-blue-600"></i> Actions</span>
                                </th>
                            </tr>
                        </thead>
                            ...
                            <tr v-for="item in items" :key="item.id" class="transition-colors hover:bg-slate-100/60"
                                ...
                                <td class="px-3 py-3 lg:px-5 lg:py-4">
                                    <div class="flex flex-col gap-1.5 text-sm">
                                        <div class="flex items-center gap-2">
                                            <span
                                                class="text-xs font-bold text-blue-700 bg-blue-100 px-2 py-1 rounded-md border border-blue-200">#{{
                                                    item.item_no }}</span>
                                            <span
                                                class="flex items-center gap-1.5 font-semibold text-slate-600 text-xs uppercase tracking-wide"><i
                                                    class="fa fa-tag text-slate-500"></i> {{ item.category?.name
                                                    }}</span>
                                        </div>
                                        <div
                                            class="text-sm font-bold text-slate-800 mt-0.5 truncate max-w-[280px] md:max-w-[350px] lg:max-w-[450px] leading-snug">
                                            {{ item.product_name }}</div>
                                        <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-slate-600 mt-1.5">
                                            <div class="flex items-center gap-1.5">
                                                <span class="flex items-center gap-1 text-slate-600"><i
                                                        class="fa fa-cubes text-slate-500 text-[10px]"></i> Qty:</span>
                                                <span class="font-semibold text-slate-800">{{ Number(item.quantity)
                                                    }}</span>
                                            </div>
                                            <div class="flex items-center gap-1.5">
                                                <span class="flex items-center gap-1 text-slate-600"><i
                                                        class="fa fa-tag text-slate-500 text-[10px]"></i> Price:</span>
                                                <span class="font-semibold text-blue-700">{{ formatCurrency(item.total)
                                                    }}</span>
                                            </div>
                                            <div class="flex flex-col gap-0.5">
                                                <div class="flex items-center gap-1.5">
                                                    <span class="flex items-center gap-1 text-slate-600"><i
                                                            class="fa fa-arrows-v text-slate-500 text-[10px]"></i>
                                                        Model:</span>
                                                ...
                                                <div class="flex items-center gap-1.5">
                                                    <span class="flex items-center gap-1 text-slate-600"><i
                                                            class="fa fa-th-large text-slate-500 text-[10px]"></i>
                                                        Type:</span>
                                                    <span class="font-semibold text-slate-800">{{
                                ...
                                <td class="px-3 py-3 lg:px-5 lg:py-4">
                                    <div class="flex flex-col gap-2 lg:gap-3 min-w-[160px] lg:min-w-[180px]">
                                        <div class="rounded-xl p-3 border border-slate-200 bg-white shadow-sm">
                                            <label
                                                class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-2"><i
                                                    class="fa fa-user text-blue-600"></i> Assign Tailor</label>
                                            <SearchableSelect :modelValue="item.tailor_id"
                                                @update:modelValue="item.tailor_id = $event" :options="tailors"
                                                placeholder="Select tailor..." input-class="assign-tailor-dropdown" />
                                        ...
                                        <div class="rounded-xl p-3 border border-slate-200 bg-white shadow-sm">
                                            <div class="flex flex-col gap-2">
                                                <div>
                                                    <label
                                                        class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-1.5"><i
                                                            class="fa fa-money text-amber-600"></i>
                                                        Commission/Item</label>
                                                    <input v-model.number="item.tailor_commission"
                                                        @input="calculateCommission(item)" type="number" step="0.01"
                                                        min="0"
                                                ...
                                                <div
                                                    ...
                                                    <span
                                                        class="flex items-center gap-1.5 text-xs font-semibold text-slate-600"><i
                                                            class="fa fa-calculator text-slate-500"></i> Total</span>
                                    ...
                                    <div class="flex flex-col gap-2 lg:gap-3 min-w-[160px] lg:min-w-[200px]">
                                        <div class="rounded-xl p-3 border border-slate-200 bg-white shadow-sm">
                                            <div class="flex items-center justify-between mb-2">
                                                <span
                                                    class="flex items-center gap-1.5 text-xs font-semibold text-slate-700"><i
                                                        class="fa fa-building text-slate-600"></i> Current Stock</span>
                                                <span
                                                    class="flex items-center gap-1 text-sm font-bold text-slate-800"><i
                                                        class="fa fa-cubes text-slate-500"></i> {{
                                                            (item.product?.stock_quantity || 0).toFixed(3) }}</span>
                                            </div>
                                            <div class="grid grid-cols-2 gap-3">
                                                <div class="flex flex-col gap-1">
                                                    <span
                                                        class="flex items-center gap-1.5 text-xs font-semibold text-slate-700"><i
                                                            class="fa fa-arrow-down text-slate-500"></i> Used</span>
                                ...
                                <td class="px-3 py-3 lg:px-5 lg:py-4">
                                    <div class="min-w-[120px] lg:min-w-[140px] flex flex-col gap-2 lg:gap-3">
                                        <div>
                                            <label
                                                class="flex items-center gap-1.5 text-xs font-semibold text-slate-700 mb-1.5"><i
                                                    class="fa fa-calendar text-blue-600"></i> Completion
                                                Date</label>
                                            <input v-model="item.item_completion_date" type="date"
                                                class="input-field w-full text-sm font-semibold py-2 px-3 rounded-lg border border-slate-300 bg-slate-50 text-slate-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:bg-white transition-all" />
                                        </div>
                                ...
                                <td class="px-3 py-3 lg:px-5 lg:py-4">
                                    <div class="flex flex-col gap-2 w-max mx-auto">
                                        <div class="flex flex-col gap-1.5">
                                            <label
                                                class="flex items-center justify-center gap-1.5 text-xs font-semibold text-slate-700"><i
                                                    class="fa fa-star-half-o text-amber-500"></i> Rating</label>
                                            <div class="flex items-center gap-1 justify-center">
                                                <button v-for="star in 5" :key="star" type="button"
                                                    @click="item.rating = star"
                                                    class="p-1.5 rounded-md transition-colors hover:bg-amber-50"
                                        ...
                                        <div v-if="item.is_selected_for_completion"
                                            ...
                                            <span
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-100 text-emerald-700 text-xs font-bold border border-emerald-200 w-full justify-center">
                                                <i class="fa fa-check-circle"></i> Ready
                                            </span>
                                        </div>
                                    </div>
