<template>
    <div class="flex flex-col gap-3">
            ...
            <div class="p-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400 text-[10px] font-medium uppercase tracking-wider">{{ items.length }}
                            items total</p>
                    </div>
                </div>
        ...
        <div v-if="items.length === 0"
            class="bg-white rounded-2xl shadow-sm border-2 border-dashed border-slate-200 py-12 text-center">
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-4 py-3 text-[0.6rem] font-bold text-slate-400 uppercase tracking-widest">Item
                                Details</th>
                            <th class="px-4 py-3 text-[0.6rem] font-bold text-slate-400 uppercase tracking-widest">
                                Tailor Assignment</th>
                            <th class="px-4 py-3 text-[0.6rem] font-bold text-slate-400 uppercase tracking-widest">
                                Material Tracking</th>
                            <th class="px-4 py-3 text-[0.6rem] font-bold text-slate-400 uppercase tracking-widest">
                                Completion Date / Qty</th>
                            <th
                                class="px-4 py-3 text-[0.6rem] font-bold text-slate-400 uppercase tracking-widest text-center">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        <tr v-for="item in items" :key="item.id" class="transition-colors hover:bg-slate-50/50"
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-col gap-2 min-w-[160px]">
                                    <div class="bg-slate-50 rounded-lg p-2 border border-slate-200">
                                        <label
                                            class="block text-[0.55rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Assign
                                            Tailor</label>
                                    ...
                                    <div class="bg-slate-50 rounded-lg p-2 border border-slate-200">
                                        <div class="flex flex-col gap-1.5">
                                            <div>
                                                <label
                                                    class="block text-[0.55rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Rate/Item</label>
                                            ...
                                            <div class="flex items-center justify-between">
                                                <span
                                                    class="text-[0.5rem] font-bold text-slate-400 uppercase tracking-wider">Total</span>
                                                <span class="text-[0.65rem] font-black text-blue-600">{{
                                ...
                                <div class="flex flex-col gap-2 min-w-[180px]">
                                    ...
                                    <div class="bg-slate-50 rounded-lg p-1.5 border border-slate-100">
                                        <div class="flex items-center justify-between mb-1.5">
                                            <span
                                                class="text-[0.5rem] font-bold text-slate-400 uppercase tracking-tighter">Current
                                                Stock</span>
                                            <span class="text-[0.65rem] font-black text-slate-700">{{
                                                (item.product?.stock_quantity || 0).toFixed(3) }}</span>
                                        </div>
                                        <div class="grid grid-cols-2 gap-2">
                                            <div class="flex flex-col gap-0.5">
                                                <span
                                                    class="text-[0.5rem] font-bold text-slate-400 uppercase">Used</span>
                                            ...
                                            <div class="flex flex-col gap-0.5">
                                                <span
                                                    class="text-[0.5rem] font-bold text-slate-400 uppercase">Waste</span>
                            ...
                            <td class="px-4 py-3">
                                <div class="min-w-[120px] flex flex-col gap-2">
                                    <div>
                                        <label
                                            class="block text-[0.55rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Completion
                                            Date</label>
                                    ...
                                    <div>
                                        <label
                                            class="block text-[0.55rem] font-bold text-slate-400 uppercase tracking-widest mb-1">Completed
                                            Qty</label>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-col gap-1.5 w-max mx-auto">
                                    <div class="flex flex-col gap-1">
                                        <label
                                            class="block text-[0.5rem] font-bold text-slate-400 uppercase tracking-wider text-center">Rating</label>
                                        <div class="flex items-center gap-1">
                                            <button v-for="star in 5" :key="star" type="button"
                                                @click="item.rating = star" class="p-1 rounded transition-colors"
                                    ...
                                    <div v-if="item.is_selected_for_completion" class="animate-[fadeIn_0.3s_ease-out]">
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-600 text-[0.55rem] font-black uppercase tracking-tighter border border-emerald-100 w-full justify-center">
