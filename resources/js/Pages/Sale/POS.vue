<template>
    <div class="min-h-screen max-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 relative overflow-hidden">
        ...
        <div class="container-fluid h-screen relative z-10 flex flex-col">
            <form class="flex-1 flex flex-col min-h-0">
                ...
                <div class="flex flex-col md:flex-row lg:flex-row flex-1 gap-1 sm:gap-2 md:gap-3 lg:gap-4 xl:gap-6 p-1 sm:p-2 md:p-3 lg:p-4 xl:p-6 min-h-0">
                    ...
                    <div class="order-1 md:order-1 w-full md:w-64 lg:w-auto md:h-full">
                        <CategoriesSidebar :categories="categories" :selected-category="selectedCategory" @category-selected="selectCategory" />
                    </div>

                    <!-- Main Content Area - Products and Cart -->
                    <div class="flex-1 flex flex-col md:flex-row lg:flex-row gap-1 sm:gap-2 md:gap-3 lg:gap-4 order-2 md:order-2 min-h-0 md:h-full">
                        ...
                        <div class="flex-1 md:flex-[0.7] lg:flex-[0.6] xl:flex-[0.55] flex flex-col order-1 md:order-1 min-h-0">
                            ...
                            <div class="bg-gradient-to-br from-white/95 via-blue-50/30 to-indigo-50/20 backdrop-blur-xl rounded-lg shadow-lg border border-white/60 mb-2 p-3 relative overflow-hidden">
                                    ...
                                    <div class="space-y-1">
                                        <label class="text-xs font-bold text-slate-700 flex items-center justify-between">
                                            ...
                                            <div class="flex items-center space-x-1">
                                                <button type="button" @click="viewCustomerDetails" :disabled="!form.account_id || form.account_id === 3"
                                                    :class="[
                                        ...
                                        <SearchableSelect v-model="form.account_id" :options="formattedCustomers" placeholder="Select Customer" filter-placeholder="Search by name or mobile..."
                                            :visibleItems="8" @search="searchCustomers" @change="handleCustomerChange"
                                ...
                                <div class="relative grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-2 mb-2 sm:mb-3">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <SearchableSelect v-model="form.sale_type" :options="priceTypes" placeholder="Select type..." filter-placeholder="Search sale types..." :visibleItems="8"
                            ...
                            <!-- Products Grid -->
                            <div class="bg-white rounded-lg shadow-md border border-slate-200 flex-1 p-2 sm:p-3 min-h-0">
                                <div class="h-full overflow-y-auto products-container custom-scrollbar"
                                    :style="{
                                        'height': windowWidth >= 1024 ? 'calc(100vh - 220px)' : windowWidth >= 768 ? 'calc(70vh - 120px)' : 'calc(50vh - 80px)',
                                        'min-height': windowWidth >= 768 ? '300px' : '200px'
                                    }">
                                    <div v-if="loading" class="flex items-center justify-center h-full">
                                        <div class="text-center">
                                            <div class="animate-spin rounded-full h-12 w-12 sm:h-16 sm:w-16 border-b-2 border-blue-500 mx-auto mb-4">
                                            </div>
                                            <p class="text-slate-600 text-sm sm:text-base">Loading products...</p>
                                        </div>
                                    </div>
                                    <products-grid v-else :products="products" :lowStockThreshold="10" @product-selected="addProductToCart"></products-grid>
                                </div>
                            </div>
                        </div>

                        <!-- Cart Section - Enhanced responsive layout -->
                        <div class="w-full md:w-80 lg:w-96 xl:w-[30rem] md:flex-[0.3] lg:flex-[0.4] xl:flex-[0.45] flex flex-col order-3 md:order-3 h-full">
                            <div class="bg-white rounded-xl shadow-lg border border-slate-200 h-full flex flex-col min-h-0">
                                <!-- Cart Items Component -->
                                <CartItems :items="form.items" :total-quantity="totalQuantity" :cart-height="cartHeight"
                                    :max-height="windowWidth >= 1024 ? '100%' : windowWidth >= 768 ? '500px' : '350px'" @view-cart-items="viewCartItems" @clear-cart="clearCart"
                                    @update-item-quantity="updateItemQuantity" @edit-cart-item="editCartItem" @remove-cart-item="removeCartItem" @increase-quantity="increaseQuantity"
                                    @decrease-quantity="decreaseQuantity" @manage-combo-offer="manageComboOffer" item-class="min-h-[64px] py-4" />
                                ...
                                <div class="p-2 sm:p-3 border-t border-slate-200">
                                    <div class="mb-2 sm:mb-3">
                                        ...
                                        <div class="relative">
                                            <input v-model.number="form.other_discount" @input="calculateTotals" type="number" step="0.01"
                                    ...
                                    <div class="bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/20 rounded-xl p-2 sm:p-3 mb-2 sm:mb-3 border border-slate-200/60 shadow-sm">
                                        <div class="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <span class="font-bold text-slate-800 bg-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm text-xs sm:text-sm">{{ form . total }}</span>
                                            </div>
                                            <div class="flex justify-between items-center text-red-600">
                                                ...
                                                <span class="font-bold bg-red-50 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm text-xs sm:text-sm">
                                                    -{{ form . other_discount }}
                                                </span>
                                            </div>
                                            ...
                                            <div class="border-t border-slate-300/60 pt-1.5 sm:pt-2 flex justify-between items-center">
                                                ...
                                                <div class="bg-gradient-to-r from-emerald-500 to-green-500 text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-xl shadow-lg">
                                    ...
                                    <div class="space-y-2 sm:space-y-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-2 sm:gap-3">
                                            ...
                                            <button type="button" @click="submitSale" :disabled="Object.keys(form.items).length === 0"
                                                :class="[
        ...
        <!-- Customer Modal -->
        <CustomerModal :show="showCustomerModal" :initial-customer="newCustomer" :customer-types="customerTypes" :countries="countries" @close="showCustomerModal = false"
            @customer-saved="handleCustomerSaved" @customer-selected="handleCustomerSelected" />

        <!-- Customer Details Modal -->
        <CustomerDetailsModal :show="showCustomerDetailsModal" :customer-id="selectedCustomerId" @close="showCustomerDetailsModal = false" @edit="handleCustomerEdit" />

        <!-- Custom Payment Modal -->
        <CustomPaymentModal :show="showCustomPaymentModal" :total-amount="form.grand_total" :payment-methods="paymentMethods" :initial-payments="customPaymentData.payments"
            @close="closeCustomPaymentModal" @save="handleCustomPaymentSave" />

        <!-- Feedback Modal -->
        <FeedbackModal :show="showFeedbackModal" :sale="form" @close="closeFeedbackModal" @feedback-submitted="handleFeedbackSubmitted" />

        <!-- Sale Confirmation Modal -->
        <SaleConfirmationModal :show="showConfirmationModal" :sale-data="confirmationData" :loading="submitting" :payment-method="selectedPaymentMethod" :send-to-whatsapp="sendToWhatsapp"
            @update:paymentMethod="selectPaymentMethod" @update:sendToWhatsapp="val => sendToWhatsapp = val" @openCustomPayment="showCustomPaymentModal = true" @close="closeConfirmationModal"
