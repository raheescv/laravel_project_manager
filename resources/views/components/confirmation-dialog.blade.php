...
<div class="confirmation-container" style="text-align: left; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
    ...
    @if ($paymentMethods && $showPaymentMethods)
        <!-- Payment Methods -->
    ...
    <div class="status-indicator"
        ...
        text-align: center;
        padding: 15px;
        background: {{ $balance === 0 ? '#f8f9fa' : ($balance > 0 ? '#fff3cd' : '#d1ecf1') }};
        border: 1px solid {{ $balance === 0 ? '#e9ecef' : ($balance > 0 ? '#ffeaa7' : '#bee5eb') }};
        border-radius: 8px;
        color: {{ $balance === 0 ? '#495057' : ($balance > 0 ? '#856404' : '#0c5460') }};
    ">
        <div
            style="
            width: 40px;
