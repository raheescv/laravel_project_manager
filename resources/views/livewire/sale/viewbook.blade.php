...
<div class="mb-4">
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover table-sm align-middle">
           ...
           <tbody>
@php
    $catIds = collect($customer_measurements)->pluck('category_id')->unique()->filter()->values()->all();
    $subcatIds = collect($customer_measurements)->pluck('sub_category_id')->unique()->filter()->values()->all();
    ...
    $grouped = collect($customer_measurements)->groupBy(function($m) {
...
@foreach ($grouped as $groupKey => $group)
    @foreach ($group as $j => $measurement)
        <tr>
            ...
            @endif
            <td><i class="fa fa-ruler me-1 text-secondary"></i> {{ $measurement['template_name'] ?? '-' }}</td>
