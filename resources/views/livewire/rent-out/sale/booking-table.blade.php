    ...
    <div class="card-header bg-white py-3">
        <div class="row g-3 align-items-center">
            <div class="col-12 col-md-6 d-flex gap-2">
                @can('rent out lease.create')
                    <a href="{{ route('property::sale::booking.create') }}" class="btn btn-primary d-inline-flex align-items-center gap-2">
                        <i class="demo-psi-add fs-5"></i>
                        <span>Add New Booking</span>
                    </a>
                @endcan
                @can('rent out lease.delete')
                    @if(count($selected) > 0)
            ...
            <div class="col-12 col-md-6">
                <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                    ...
                    <div class="d-flex bg-light rounded-2 px-2 flex-grow-1 flex-md-grow-0" style="min-width: 250px;">
                        ...
                        <input type="text" wire:model.live="search" autofocus placeholder="Search bookings..." class="form-control bg-transparent border-0 py-2" style="box-shadow: none; font-size: 0.875rem;" autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                    ...
                    <tr>
                        <th class="ps-4" width="80">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" wire:model.live="selectAll" id="saleBookingSelectAll">
                            </div>
                        </th>
                        ...
                        <th> <x-sortable-header field="rent" label="Total" :sortField="$sortField" :direction="$sortDirection" /> </th>
                        <th>Status</th>
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                @can('rent out lease.view')
                                    <a href="{{ route('property::sale::booking.view', $item->id) }}" class="btn btn-icon btn-sm btn-hover btn-light" title="View Booking">
                                        <i class="demo-psi-file fs-5 text-muted"></i>
                                    </a>
                                @endcan
                                @can('rent out lease.edit')
                                    <a href="{{ route('property::sale::booking.create', $item->id) }}" class="btn btn-icon btn-sm btn-hover btn-light" title="Edit Booking">
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="text-center py-5 text-muted">No sale bookings found matching your search.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
