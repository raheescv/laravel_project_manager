        ...
        <div class="row g-3 align-items-center">
            <div class="col-12 col-md-6 d-flex gap-2">
                @can($config->createPermission)
                    <a href="{{ route($config->createRoute) }}" class="btn btn-primary d-inline-flex align-items-center gap-2">
            ...
            <div class="col-12 col-md-6">
                <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                    ...
                    <div class="d-flex bg-light rounded-2 px-2 flex-grow-1 flex-md-grow-0" style="min-width: 250px;">
                        ...
                        <input type="text" wire:model.live="search" autofocus placeholder="{{ $config->searchPlaceholder }}" class="form-control bg-transparent border-0 py-2" style="box-shadow: none; font-size: 0.875rem;" autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                @can($config->viewPermission)
                                    <a href="{{ route($config->viewRoute, $item->id) }}" class="btn btn-icon btn-sm btn-hover btn-light" title="View">
                                        <i class="demo-psi-file fs-5 text-muted"></i>
                                    </a>
                                @endcan
                                @can($config->editPermission)
                                    <a href="{{ route($config->createRoute, $item->id) }}" class="btn btn-icon btn-sm btn-hover btn-light" title="Edit {{ $config->singularLabel }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="text-center py-5 text-muted">{{ $config->emptyMessage }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
