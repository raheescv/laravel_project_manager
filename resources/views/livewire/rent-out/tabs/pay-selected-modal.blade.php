                ...
                <div class="row g-2 g-md-3 align-items-end">
                    <div class="col-6 col-md-3">
                        <label class="form-label fw-semibold small mb-1">
                            <i class="fa fa-calendar me-1 text-muted"></i> Payment Date
                        </label>
                        <input type="date"
                        ...
                        @forelse($cashTerms as $index => $ct)
                            <tr>
                                ...
                                <td class="py-1 text-end">
                                    <span
                                        class="fw-bold text-danger">{{ number_format($ct['balance'], 2) }}</span>
                                </td>
                                <td class="py-1">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">
                                    ...
                                    <span class="small">No pending payment terms selected</span>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div class="card border mb-2 shadow-sm">
                    <div class="card-body p-2">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span
                                class="fw-bold text-danger small">{{ number_format($ct['balance'], 2) }}</span>
                        </div>
                        @if ($ct['customer'] || $ct['property'])
                            <div class="small text-muted mb-2">
                                {{ $ct['customer'] }}
                                @if ($ct['property'])
                                    <span class="ms-1 fw-medium text-dark">&bull;
                                        {{ $ct['property'] }}</span>
                                @endif
                            </div>
                        @endif
                        <div class="row g-2">
                            <div class="col-6">
                                <label class="form-label text-muted mb-0"
                                    style="font-size:.7rem;">Mode</label>
                                <select class="form-select form-select-sm border-secondary-subtle"
                                    wire:model="cashTerms.{{ $index }}.payment_mode">
                                    @foreach ($paymentMethods as $value => $label)
                                        <option value="{{ $value }}">{{ $label }}
                            ...
                            <div class="col-6">
                                <label class="form-label text-muted mb-0"
                                    style="font-size:.7rem;">Amount</label>
                                <input type="number"
                                    class="form-control form-control-sm text-end border-secondary-subtle fw-medium"
                            ...
                            <div class="col-12">
                                <label class="form-label text-muted mb-0"
                                    style="font-size:.7rem;">Remark</label>
                                <input type="text"
                                    class="form-control form-control-sm border-secondary-subtle"
                                    wire:model="cashTerms.{{ $index }}.remark"
                                    placeholder="Remark...">
                ...
                <div class="card border-0 bg-light mt-2">
                    <div class="card-body p-2 d-flex justify-content-between small fw-bold">
                        <span>Total Balance: <span
                                class="text-danger">{{ number_format($this->balanceTotal, 2) }}</span></span>
                        <span>Paying: <span
                                style="color:#2e7d56;">{{ number_format($this->payingTotal, 2) }}</span></span>
                    </div>
                </div>
            @endif
        </div>
