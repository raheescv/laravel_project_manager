        ...
        <div class="row g-3 align-items-end">
            <div class="col-12 col-xl-6">
                <div class="d-flex flex-wrap gap-2">
                    ...
                    <button type="button" class="btn btn-sm btn-dark rounded-pill px-3 shadow-sm" onclick="window.print()">
        ...
        <div class="card border-0 shadow-sm rounded-4 position-relative z-3">
            <div class="card-body p-3 p-md-4 bg-white position-relative">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
                    <div class="d-flex flex-wrap gap-2">
                        <span class="badge rounded-pill text-bg-warning text-dark px-3 py-2"><i class="fa fa-filter me-1"></i>Advanced Filters</span>
                        <span class="badge rounded-pill text-bg-info px-3 py-2"><i class="fa fa-search me-1"></i>Live Search Enabled</span>
                    </div>
                    <span class="badge rounded-pill text-bg-light border px-3 py-2 text-secondary">
                        <i class="fa fa-sliders me-1"></i>Smart Controls
                    </span>
                </div>
                        ...
                        <tr>
                            ...
                            @if ($visibleColumns['order_no'] ?? true)
                                <td class="text-nowrap">
                                    <a href="{{ route('tailoring::order::show', $item->id) }}" class="text-primary fw-semibold text-decoration-none"><i class="fa fa-file-text-o me-1 small"></i>{{ $item->order_no }}</a>
                                </td>
                            @endif
                            @if ($visibleColumns['order_date'] ?? true)
                            ...
                            @endif
                            @if ($visibleColumns['bill_amount'] ?? true)
                                <td class="text-end"><i class="fa fa-money me-1 text-muted"></i>{{ currency((float) ($item->bill_amount ?? 0)) }}</td>
                            @endif
                            @if ($visibleColumns['paid_amount'] ?? true)
                                <td class="text-end"><i class="fa fa-check-circle me-1 text-success"></i>{{ currency((float) ($item->paid_amount ?? 0)) }}</td>
                            @endif
                            @if ($visibleColumns['balance_amount'] ?? true)
                                <td class="text-end"><i class="fa fa-money me-1 text-warning"></i>{{ currency((float) ($item->balance_amount ?? 0)) }}</td>
                            @endif
                            @if ($visibleColumns['item_quantity'] ?? true)
                                <td class="text-end"><i class="fa fa-cubes me-1 text-muted"></i>{{ round($item->item_quantity) }}</td>
                            @endif
                            @if ($visibleColumns['completed_qty'] ?? true)
                                <td class="text-end"><i class="fa fa-check me-1 text-success"></i>{{ round($item->completed_qty) }}</td>
                            @endif
                            @if ($visibleColumns['pending_qty'] ?? true)
                                <td class="text-end"><i class="fa fa-clock-o me-1 text-warning"></i>{{ round($item->pending_qty) }}</td>
                            @endif
                            @if ($visibleColumns['delivery_qty'] ?? true)
                                <td class="text-end"><i class="fa fa-truck me-1 text-info"></i>{{ round($item->delivery_qty) }}</td>
                            @endif
                            @if ($visibleColumns['order_status'] ?? true)
                                <td>
                                    @php $s = $item->order_status; @endphp
                                    <span class="badge bg-{{ $s === 'delivered' || $s === 'completed' ? 'success' : ($s === 'pending' ? 'warning' : 'secondary') }} bg-opacity-10 text-{{ $s === 'delivered' || $s === 'completed' ? 'success' : ($s === 'pending' ? 'warning' : 'secondary') }}">
