<div class="card border-0 shadow-sm" id="category-model-types-card">
        ...
        <div class="row g-3 align-items-center">
            <div class="col-12 col-md-6 d-flex gap-2 flex-wrap">
                @can('tailoring category.create')
                    <button type="button" class="btn btn-primary d-inline-flex align-items-center gap-2" id="TailoringCategoryModelTypeAdd" wire:click="$dispatch('TailoringCategoryModelType-Page-Create-Component', { tailoring_category_id: @js($categoryId) })">
            ...
            <div class="col-12 col-md-6">
                <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                    ...
                    <div class="d-flex bg-light rounded-2 px-2 flex-grow-1 flex-md-grow-0" style="min-width: 200px;">
                        ...
                        <input type="text" wire:model.live="search" placeholder="Search model types..." class="form-control bg-transparent border-0 py-2" style="box-shadow: none; font-size: 0.875rem;" autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card-body p-0">
        @if(!$categoryId)
            <div class="text-center py-5 text-muted">
                <i class="fa fa-tags fs-1 opacity-50"></i>
                <p class="mb-0 mt-2">Select a category above to view and manage its model types.</p>
            </div>
        @else
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                        ...
                        <tr>
                            <th class="ps-4" width="80">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model.live="selectAll" id="selectAllModelTypes">
                                </div>
                            </th>
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    @can('tailoring category.edit')
                                        <button type="button" table_id="{{ $item->id }}" class="btn btn-icon btn-sm btn-hover btn-light edit-model-type" title="Edit Model Type">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-5 text-muted">No model types found for this category.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                $(document).on('click', '.edit-model-type', function() {
                    Livewire.dispatch("TailoringCategoryModelType-Page-Update-Component", {
                        id: $(this).attr('table_id')
                    });
                });
                window.addEventListener('RefreshTailoringCategoryModelTypeTable', event => {
                    Livewire.dispatch("RefreshTailoringCategoryModelTypeTable");
                });
            });
        </script>
    @endpush
