    ...
    <div class="card shadow-sm border-0 import-wizard">
        ...
        <div class="card-header bg-body-tertiary border-bottom py-3">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                <a href="{{ route('settings::checklist_item::index') }}" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center shadow-sm">
                    <i class="fa fa-arrow-left me-2"></i> Back to Checklist Items
                </a>
                ...
                <div class="row g-4 align-items-stretch">
                    ...
                    <div class="col-lg-5">
                        <label class="fw-semibold small text-uppercase text-muted mb-2 d-block">
                            <i class="fa fa-cogs me-1"></i> On Duplicate Item
                        </label>
                        ...
                        <div class="text-muted mb-3" style="font-size: 11px;">
                            ...
                            Matched by <strong>Name + Category + Property Type</strong>.
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="hiw-card d-flex gap-3 h-100">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-muted small mb-0">Headers like <strong>Item</strong>, <strong>Room</strong>, <strong>Section</strong> are auto-matched. Review &amp; adjust next.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="hiw-card d-flex gap-3 h-100">
                                    <span class="hiw-icon text-success bg-success bg-opacity-10"><i class="fa fa-home"></i></span>
                                    <div>
                                        <div class="fw-bold small mb-1">Auto-Create Property Types</div>
                                        <p class="text-muted small mb-0">Property types referenced by name are <strong>created automatically</strong>. Leave the column blank for universal items.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="hiw-card d-flex gap-3 h-100">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-muted small mb-0">Existing items (matched by <strong>name + category + property type</strong>) are skipped or updated per your choice.</p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                    ...
                    <div class="col-lg-4">
                        <div class="card bg-body-tertiary border-0 sticky-top" style="top: 20px;">
                            <div class="card-body p-3 text-center">
                                ...
                                <p class="text-muted small mb-2">Match your spreadsheet columns to checklist item fields. Fields marked <strong>(*)</strong> are required.</p>
            ...
            @if ($step == 3)
                ...
                <div class="alert {{ $duplicateStrategy === 'update' ? 'alert-success' : 'alert-info' }} mt-3 mb-0 small d-flex align-items-center">
                    ...
                    <span>
                        ...
                        Existing items matched by <em>name + category + property type</em> will be {{ $duplicateStrategy === 'update' ? 'updated with the new values' : 'skipped' }}.
                    </span>
                </div>
            ...
            @if ($step == 4)
                <div class="text-center py-5" wire:poll.5s="checkJobStatus">
                    ...
                    @elseif ($importStatus === 'completed')
                        ...
                        <p class="text-muted mb-3">All checklist items imported successfully.</p>
                    @else
                        <div class="mb-3" id="cli-spinner">
                            <div class="spinner-grow text-success" role="status" style="width: 4rem; height: 4rem;"></div>
                        </div>
                        <div class="mb-3 d-none" id="cli-success-icon">
                            <i class="fa fa-check-circle text-success" style="font-size: 4rem;"></i>
                        </div>
                        <div class="mb-3 d-none" id="cli-error-icon">
                            <i class="fa fa-times-circle text-danger" style="font-size: 4rem;"></i>
                        </div>

                        <h4 class="fw-bold" id="cli-status-title">Import in Progress</h4>
                        <p class="text-muted mb-4" id="cli-status-message">Processing your checklist items in the background. You can leave this page.</p>

                        <div class="progress mx-auto shadow-sm mb-2" style="height: 24px; max-width: 500px; border-radius: 12px;" id="cli-progress-container">
                            <div class="progress-bar progress-bar-striped progress-bar-animated bg-success fw-bold" id="cli-progress-bar" role="progressbar" style="width: 0%"
                                aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">0%</div>
                        </div>

                        <div class="alert alert-danger mx-auto mt-3 d-none text-start" style="max-width: 500px;" id="cli-error-details">
                            <i class="fa fa-exclamation-triangle me-1"></i>
                            <span id="cli-error-text"></span>
                            <p class="mb-0 mt-1 small">Check your notifications for the detailed error report.</p>
                        </div>
                    @endif

                    <div class="mt-4">
                        <a href="{{ route('settings::checklist_item::index') }}" class="btn btn-outline-primary px-4">
                            Go to Checklist Items
                        </a>
                    </div>
                </div>
            @endif
...
@push('scripts')
    ...
    <script>
        let cliFileImportProgressInitialized = false;
        function initChecklistItemFileImportProgress() {
            if (cliFileImportProgressInitialized) return;
            cliFileImportProgressInitialized = true;
            ...
            channel.bind('file-import-event-{{ auth()->id() }}', function(data) {
                if (data.type === 'ChecklistItem') {
                    ...
                    const successIcon = document.getElementById('cli-success-icon');
                    const errorIcon = document.getElementById('cli-error-icon');
                    const statusTitle = document.getElementById('cli-status-title');
                    const statusMessage = document.getElementById('cli-status-message');
                    const progressContainer = document.getElementById('cli-progress-container');
                    const errorDetails = document.getElementById('cli-error-details');
                    ...
                    if (data.progress === -1) {
                        ...
                        if (data.progress >= 100) {
                            ...
                            if (statusMessage) statusMessage.textContent = 'All checklist items imported. Redirecting...';
                            if (progressBar) progressBar.classList.remove('progress-bar-animated');

                            setTimeout(() => {
                                window.location.href = "{{ route('settings::checklist_item::index') }}";
                            }, 2000);
                        }
                    }
                }
        ...
        document.addEventListener('livewire:initialized', initChecklistItemFileImportProgress);
        if (window.Livewire) {
            initChecklistItemFileImportProgress();
        }
    </script>
@endpush
