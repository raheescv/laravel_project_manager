    ...
    <div class="card-header bg-white py-3">
        <div class="row g-3 align-items-center">
            <div class="col-12 col-md-6 d-flex gap-2">
                @can('rack.create')
                    <button class="btn btn-primary d-inline-flex align-items-center gap-2" id="RackAdd">
            ...
            <div class="col-12 col-md-6">
                <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                    ...
                    <div class="d-flex bg-light rounded-2 px-2 flex-grow-1 flex-md-grow-0" style="min-width: 250px;">
                        ...
                        <input type="text" wire:model.live="search" autofocus placeholder="Search racks..." class="form-control bg-transparent border-0 py-2" style="box-shadow: none; font-size: 0.875rem;" autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th width="10%"> <x-sortable-header field="is_active" label="Status" :sortField="$sortField" :direction="$sortDirection" /> </th>
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                @can('rack.edit')
                                    <button table_id="{{ $item->id }}" class="btn btn-icon btn-sm btn-hover btn-light edit" title="Edit Rack">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">No racks found matching your search.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <script>
            $(document).ready(function() {
                $(document).on('click', '.edit', function() {
                    Livewire.dispatch("Rack-Page-Update-Component", {
                        id: $(this).attr('table_id')
                    });
                });
                $('#RackAdd').click(function() {
                    Livewire.dispatch("Rack-Page-Create-Component");
                });
                window.addEventListener('RefreshRackTable', event => {
                    Livewire.dispatch("RefreshRackTable");
                });
            });
        </script>
    @endpush
