<div>
    <table class="table table-striped align-middle table-sm table-bordered">
        <tbody>
            @if($general_voucher_visible_column)
                @foreach ($general_voucher_visible_column as $column => $visible)
                    <tr>
                        <th>
                            <label class="me-3">
        ...
        <tfoot>
            <tr>
                <th> <a class="btn btn-info" href="{{ route('account::general-voucher::index') }}">Apply</a> </th>
            </tr>
        </tfoot>
    </table>
</div>
