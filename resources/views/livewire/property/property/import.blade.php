        ...
        <div class="card-header bg-light py-3">
            <div class="row align-items-center">
                <div class="col-md-6 d-flex flex-wrap gap-2 align-items-center mb-3 mb-md-0">
                    <a href="{{ route('property::property::index') }}" class="btn btn-primary d-flex align-items-center shadow-sm">
        ...
        <div class="card-body">
            ...
            @if ($step == 1)
                <div class="text-center py-5">
                    ...
                    <div class="mt-4 mx-auto text-start" style="max-width: 700px;">
                        <div class="alert alert-info mb-0">
                            <strong><i class="fa fa-info-circle me-1"></i> Tip:</strong>
                            Building, Property Type and Group columns can contain either the ID or the exact name as
                            stored in the system. Use <code>upload_type=update</code> to modify existing properties
                            (matched by Building + Number).
                        </div>
                    </div>
                </div>
            @endif
                    ...
                    <div class="col-md-4">
                        <div class="card bg-light border-0 sticky-top" style="top: 20px;">
                            <div class="card-body p-3 text-center">
                                ...
                                <p class="text-muted small">Match your spreadsheet columns to property fields. Auto-matched headers are pre-selected.</p>
                                <hr>
            ...
            @if ($step == 4)
                <div class="text-center py-5">
                    ...
                    <p class="text-muted mb-4">Processing your properties in the background. You can leave this page.</p>

                    <div class="progress mx-auto shadow-sm mb-2" style="height: 24px; max-width: 500px; border-radius: 12px;">
                        <div class="progress-bar progress-bar-striped progress-bar-animated bg-success fw-bold" id="property-progress-bar" role="progressbar" style="width: 0%"
                            aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">0%</div>
                    </div>

                    <div class="mt-4">
                        <a href="{{ route('property::property::index') }}" class="btn btn-outline-primary px-4">
                            Go to Property List
                        </a>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        let propertyImportProgressInitialized = false;
        function initPropertyImportProgress() {
            if (propertyImportProgressInitialized) return;
            propertyImportProgressInitialized = true;
            ...
            channel.bind('file-import-event-{{ auth()->id() }}', function(data) {
                if (data.type === 'Property') {
                    const progressBar = document.getElementById('property-progress-bar');
                    if (progressBar) {
                        ...
                        if (data.progress >= 100) {
                            setTimeout(() => {
                                alert('Property import completed successfully!');
                                window.location.href = "{{ route('property::property::index') }}";
                            }, 1000);
                        }
                    }
                }
        ...
        document.addEventListener('livewire:initialized', initPropertyImportProgress);
        if (window.Livewire) {
            initPropertyImportProgress();
        }
    </script>
@endpush
