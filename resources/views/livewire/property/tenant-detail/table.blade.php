    ...
    <div class="card-header bg-white py-3">
        <div class="row g-3 align-items-center">
            <div class="col-12 col-md-6 d-flex gap-2">
                @can('tenant detail.create')
                    <button class="btn btn-primary d-inline-flex align-items-center gap-2" id="TenantDetailAdd">
                        <i class="demo-psi-add fs-5"></i>
                        <span>Add New</span>
                    </button>
                @endcan
                @can('tenant detail.delete')
                    @if(count($selected) > 0)
            ...
            <div class="col-12 col-md-6">
                <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                    ...
                    <div class="d-flex bg-light rounded-2 px-2 flex-grow-1 flex-md-grow-0" style="min-width: 250px;">
                        ...
                        <input type="text" wire:model.live="search" autofocus placeholder="Search tenant details..." class="form-control bg-transparent border-0 py-2" style="box-shadow: none; font-size: 0.875rem;" autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th width="8%"> <x-sortable-header field="id" label="ID" :sortField="$sortField" :direction="$sortDirection" /> </th>
                        <th>Property</th>
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                @can('tenant detail.edit')
                                    <button table_id="{{ $item->id }}" class="btn btn-icon btn-sm btn-hover btn-light edit" title="Edit Tenant Detail">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-5 text-muted">No tenant details found matching your search.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <script>
            $(document).ready(function() {
                $(document).on('click', '.edit', function() {
                    Livewire.dispatch("TenantDetail-Page-Update-Component", {
                        id: $(this).attr('table_id')
                    });
                });
                $('#TenantDetailAdd').click(function() {
                    Livewire.dispatch("TenantDetail-Page-Create-Component");
                });
                window.addEventListener('RefreshTenantDetailTable', event => {
                    Livewire.dispatch("TenantDetail-Refresh-Component");
                });
            });
        </script>
    @endpush
