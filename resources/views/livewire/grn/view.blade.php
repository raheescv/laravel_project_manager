<div>
    ...
    {{--
        ╔══════════════════════════════════════════════════════════════════════╗
        ║  Goods Received Note — View · "Premium Compact" design system         ║
    ...
    @php
        ...
        $vendorName = $grn->vendor?->name ?? $grn->localPurchaseOrder?->vendor?->name ?? '—';
        $itemCount = $grn->items->count();
        $totalQty = $grn->items->sum('quantity');
        $totalValue = $grn->items->sum(fn($i) => $i->quantity * ($i->localPurchaseOrderItem?->rate ?? 0));
    @endphp

    <div class="lpox" data-status="{{ $grn->status->value }}">

        {{-- ===================== HERO ===================== --}}
        <div class="lpox-hero">
            <div class="glow"></div>
            <div class="lpox-hero-inner">
                <div class="doc-ic"><i class="fa fa-cubes"></i></div>
                <div class="h-main">
                    <div class="h-eyebrow">Goods Received Note</div>
                    <div class="h-ref">{{ $grn->grn_no }}</div>
                    <div class="h-meta">
                        <span><i class="demo-psi-home"></i>{{ $grn->branch?->name ?? '—' }}</span>
                        <span><i class="demo-psi-shop"></i>{{ $vendorName }}</span>
                        <span><i class="demo-psi-calendar-4"></i>{{ $grn->date ? \Carbon\Carbon::parse($grn->date)->format('d M Y') : '—' }}</span>
                        @if ($grn->localPurchaseOrder)
                            <span><i class="demo-psi-shopping-cart"></i><a href="{{ route('lpo::view', $grn->localPurchaseOrder->id) }}">LPO #{{ $grn->localPurchaseOrder->id }}</a></span>
                        @endif
                    </div>
                </div>
                <div class="h-right">
                    <span class="status-pill">
                        ...
                        <i class="{{ $statusIcon }}"></i> {{ strtoupper($grn->status->label()) }}
                    </span>
                    @if ($grn->status === GrnStatus::PENDING)
                        @can('grn.create')
                            <a href="{{ route('grn::edit', $grn->id) }}" class="btn-ghost">
                                <i class="fa fa-pencil"></i> Edit
                            </a>
                        @endcan
                        @can('grn.decide')
                            <a href="{{ route('grn::decision', $grn->id) }}" class="btn-print">
            ...
            <div class="lpox-hstats">
                ...
                <div class="lpox-hs">
                    <div class="s-k"><i class="fa fa-sort-amount-asc"></i> Received Qty</div>
                    <div class="s-v">{{ number_format($totalQty, 0) }} <small>units</small></div>
                </div>
                <div class="lpox-hs">
                    <div class="s-k"><i class="demo-psi-coin"></i> Received Value</div>
                    <div class="s-v">{{ number_format($totalValue, 2) }}</div>
                </div>
                @if ($grn->journals->count())
                    <div class="lpox-hs">
                        <div class="s-k"><i class="fa fa-book"></i> Journals</div>
                        <div class="s-v">{{ $grn->journals->count() }} <small>{{ Str::plural('entry', $grn->journals->count()) }}</small></div>
                    </div>
                @endif
            </div>
        </div>
        ...
        @php
            ...
            $timeline[] = [
                ...
                'icon' => 'demo-psi-file',
                'tone' => 'acc',
                'who' => $grn->creator?->name,
                'note' => $grn->remarks,
                'note_label' => 'Remarks',
            ];

            // 2. Accepted / Rejected decision
            if ($grn->decision_at && in_array($grn->status, [GrnStatus::ACCEPTED, GrnStatus::REJECTED])) {
                $isReject = $grn->status === GrnStatus::REJECTED;
                $timeline[] = [
                    'ts' => $grn->decision_at,
                    ...
                    'who' => $grn->decisionMaker?->name,
                    'note' => $grn->decision_note,
            ...
            if ($grn->updated_at && $lastEventTs && $grn->updated_at->gt($lastEventTs->copy()->addMinute())) {
                $timeline[] = [
                    'ts' => $grn->updated_at,
                    'title' => 'Last Updated',
                    'badge' => 'Edited',
        ...
        <div class="lpox-grid">
            ...
            <div class="l-card" style="margin-bottom:0">
                <div class="l-head">
                    ...
                    <div><div class="l-title">GRN Information</div><div class="l-sub">Vendor &amp; receipt metadata</div></div>
                </div>
                <div class="kv-grid">
                    <div class="kv"><div class="kk"><i class="demo-psi-tag"></i>GRN No</div><div class="vv">{{ $grn->grn_no }}</div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-calendar-4"></i>Date</div><div class="vv">{{ $grn->date ? systemDate($grn->date) : '-' }}</div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-shop"></i>Vendor</div><div class="vv">{{ $vendorName }}</div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-shopping-cart"></i>LPO</div><div class="vv">
                        @if ($grn->localPurchaseOrder)
                            <a href="{{ route('lpo::view', $grn->localPurchaseOrder->id) }}">LPO #{{ $grn->localPurchaseOrder->id }} <i class="fa fa-external-link-alt" style="font-size:9px"></i></a>
                        @else
                            -
                        @endif
                    </div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-male"></i>Created By</div><div class="vv">{{ $grn->creator?->name ?? '-' }}</div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-home"></i>Branch</div><div class="vv">{{ $grn->branch?->name ?? '-' }}</div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-basket-coins"></i>Total Items</div><div class="vv">{{ $itemCount }} items</div></div>
                    <div class="kv"><div class="kk"><i class="demo-psi-coin"></i>Received Value</div><div class="vv">{{ number_format($totalValue, 2) }}</div></div>
                    @if ($grn->remarks)
                        <div class="kv full"><div class="kk"><i class="demo-psi-speech-bubble-3"></i>Remarks</div><div class="vv">{{ $grn->remarks }}</div></div>
                    @endif
                </div>
            </div>
        ...
        <div class="l-sec l-card">
            <div class="l-head">
                ...
                <div style="flex:1"><div class="l-title">Received Items</div></div>
                <span class="l-pill pill-acc">{{ $itemCount }} items · {{ number_format($totalValue, 2) }}</span>
            </div>
            <div class="l-tblwrap">
                <table class="l-tbl">
                    <thead>
                    ...
                    <tbody>
                        @php $totalRate = 0; @endphp
                        @forelse ($grn->items as $index => $item)
                            @php
                                $rate = $item->quantity * ($item->localPurchaseOrderItem?->rate ?? 0);
                                $totalRate += $rate;
                            @endphp
                            <tr>
                                ...
                                <td class="num"><span class="pname" style="color:var(--ok)">{{ $item->quantity }}</span></td>
                                <td class="num"><span class="psub">{{ number_format($item->localPurchaseOrderItem?->rate ?? 0, 2) }}</span></td>
                                <td class="num amt">{{ number_format($rate, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7">
                                    <div class="l-empty"><i class="demo-psi-basket-coins"></i> No items received</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                    @if ($itemCount)
                        <tfoot>
                            <tr class="tfoot-acc">
                                ...
                                <td></td>
                                <td class="num">{{ number_format($totalRate, 2) }}</td>
                            </tr>
                        </tfoot>
                    @endif
                </table>
        ...
        @if ($grn->journals->count())
            ...
            <div class="l-sec l-card">
                <div class="l-head">
                    ...
                    <div style="flex:1"><div class="l-title">Journal Entries</div><div class="l-sub">Accounting postings for this GRN</div></div>
                    <span class="l-pill pill-purple">{{ $grn->journals->count() }} {{ Str::plural('journal', $grn->journals->count()) }}</span>
                </div>
                @foreach ($grn->journals as $journal)
                    @php $filteredEntries = $journal->entries->where('account_id', '!=', $vendorId); @endphp
                    <div class="jnl">
                        <div class="jnl-head">
                            <div>
            ...
            <div class="lpox-action tone-warn">
                <div class="a-body">
                    <div class="a-head">
                        ...
                        <div><div class="a-title">Take Action</div><div class="a-sub">Accept or reject this goods received note</div></div>
                    </div>
                    ...
                    <div class="a-actions">
                        <button type="button" class="l-btn l-btn-bad" wire:click="reject" wire:confirm="Are you sure you want to reject this GRN?">
                            <i class="fa fa-times"></i> Reject
                        </button>
                        <button type="button" class="l-btn l-btn-ok" wire:click="accept" wire:confirm="Are you sure you want to accept this GRN?">
