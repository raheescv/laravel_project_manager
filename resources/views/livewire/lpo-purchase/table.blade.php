<div x-data="{
    ...
    items: @js($this->purchases->pluck('id')),
    selectAll: false,
    ...
    <div class="bg-white card-header">
        <div class="row g-3">
            <div class="col-md-4 d-flex align-items-center">
                <div class="btn-group">
                    @can('lpo-purchase.create')
                        <a class="btn btn-primary d-flex align-items-center shadow-sm" href="{{ route('lpo-purchase::create') }}">
                            <i class="demo-psi-add me-2"></i>
                            Create
                        </a>
                    @endcan
                    @can('lpo-purchase.delete')
                    ...
                    <div class="form-group" style="width: 250px;">
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" wire:model.live.debounce.500ms="search" class="form-control border-start-0"
                                placeholder="Search purchases..." autofocus>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-3 rounded bg-light">
                <div class="row g-3">
                    ...
                    <div class="col-md-3" wire:ignore>
                        ...
                        {{ html()->select('status', PurchaseStatus::values())->value($status)->class('form-control form-control-sm')->id('status')->placeholder('All Statuses') }}
                    </div>
                    <div class="col-md-3" wire:ignore>
            ...
            <table class="table mb-0 align-middle table-striped table-hover table-sm border-bottom">
                <thead class="bg-light text-nowrap">
                    <tr>
                        ...
                        <th> LPO </th>
                        <th> <x-sortable-header :direction="$sortDirection" :sortField="$sortField" field="account_id" label="Vendor" /> </th>
                        <th> Items </th>
                    ...
                    @foreach ($this->purchases as $item)
                        <tr>
                            ...
                            <td class="text-nowrap">
                                <a href="{{ route('lpo-purchase::view', $item->id) }}" class="fw-medium">
                                    {{ $item->invoice_no ?? '-' }}
                                </a>
                            </td>
                            <td class="text-nowrap">
                            ...
                            <td>
                                ...
                                @endphp
                                <div class="badge bg-{{ $statusColor }} bg-opacity-10 text-{{ $statusColor }}">
                                    {{ ucfirst($item->status) }}
                                </div>
                            </td>
                            <td class="text-nowrap">
                            ...
                            <td class="text-nowrap">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('lpo-purchase::view', $item->id) }}">
                                        ...
                                        @if ($item->status === 'pending')
                                            @can('lpo-purchase.create')
                                                <li>
                                                    <a class="dropdown-item" href="{{ route('lpo-purchase::edit', $item->id) }}">
                                            ...
                                            @can('lpo-purchase.decide')
                                                <li>
                                                    <a class="dropdown-item" href="{{ route('lpo-purchase::decision', $item->id) }}">
