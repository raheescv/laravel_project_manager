<x-app-layout>
        ...
        <div class="content__wrap">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">{{ $breadcrumb ?? 'Rent' }}</li>
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-end gap-2 mt-2">
                <div>
                    <h1 class="page-title mb-0">{{ $title ?? 'RentOut Service Report' }}</h1>
                    <p class="lead mb-0">{{ $subtitle ?? 'Track service charges, payments and outstanding balances across all rent agreements.' }}</p>
                </div>
            </div>
        </div>
    </div>
    <div class="content__boxed">
        <div class="content__wrap">
            @livewire('rent-out.service-payment-table', ['agreementType' => $agreementType ?? 'rental'])
        </div>
    </div>
    @push('scripts')
