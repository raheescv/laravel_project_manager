...
class JournalEntryAction
{
    ...
    public function execute($purchase, $userId)
    {
        try {
            ...
            if (empty($accounts['grn_clearing']) || empty($accounts['tax_amount']) || empty($accounts['discount']) || empty($accounts['freight'])) {
                throw new \Exception('Required account heads are missing for purchase journal posting.');
            }

            $entries = [];

            // Invoice Entry (settles GRN clearing to vendor)
            if ($purchase->gross_amount > 0) {
                $remarks = 'Purchase from '.$purchase->account->name;
                $debit = $purchase->gross_amount;
                $credit = 0;
                $entries[] = $this->makeEntryPair($accounts['grn_clearing'], $purchase->account_id, $debit, $credit, $remarks, 'Purchase', $purchase->id);
            }
