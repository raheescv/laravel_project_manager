...
class CustomerVisitHistory extends Component
{
    ...
    public function mount()
    {
        ...
        $this->accounts = Account::select('id', 'name')->get();
    }
    ...
    public function render()
    {
        $visits = Sale::query()
            ...
            ->selectRaw('CASE WHEN (SELECT MIN(date) FROM sales WHERE account_id = accounts.id) BETWEEN ? AND ? THEN true ELSE false END as is_new_customer', [$this->from_date, $this->to_date])->when($this->accoun, fn ($q) => $q->where('account_id', $this->accoun))
