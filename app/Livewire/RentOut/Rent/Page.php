...
{
    public $type = 'Rentout'; // Rentout or Booking
    ...
    public function loadData()
    {
        if ($this->table_id) {
            ...
            if (! $item) {
                ...
                return redirect()->route('property::rent::index');
            }
            $this->rentouts = $item->toArray();
            $this->rentouts['agreement_type'] = $item->agreement_type?->value ?? 'rental';
            ...
            $this->rentouts = [
                'agreement_type' => AgreementType::Rental->value,
                'account_id' => '',
                'property_group_id' => '',
    ...
    public function confirm()
    {
        try {
            ...
            return redirect()->route('property::rent::view', $this->table_id);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function save()
    {
        try {
            ...
            if ($this->type === 'Booking') {
                return redirect()->route('property::rent::booking.create', $this->table_id);
            }

            return redirect()->route('property::rent::create', $this->table_id);
        } catch (\Exception $e) {
            DB::rollback();
            $this->dispatch('error', ['message' => $e->getMessage()]);
        }
    ...
    {
        return view('livewire.rent-out.rent.page');
    }
}
