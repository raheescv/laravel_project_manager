...
class Table extends Component
{
    ...
    protected $listeners = [
        'RentOutRent-Refresh-Component' => '$refresh',
    ];
    ...
    public function delete()
    {
        try {
            ...
            $this->dispatch('RefreshRentOutRentTable');
        } catch (\Exception $e) {
            DB::rollback();
    ...
    {
        if ($value) {
            $this->selected = RentOut::rental()->latest()->limit(2000)->pluck('id')->toArray();
        } else {
            $this->selected = [];
        }
    }
    ...
    public function render()
    {
        $data = RentOut::with(['customer', 'property', 'building'])
            ->where('agreement_type', AgreementType::Rental)
        ...
        return view('livewire.rent-out.rent.table', [
            'data' => $data,
        ]);
    }
}
