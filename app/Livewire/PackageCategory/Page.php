...
class Page extends Component
{
    ...
    protected function validationAttributes()
    {
        return [
            ...
            'package_categories.frequency' => 'frequency',
            'package_categories.no_of_visits' => 'no of visits',
        ];
    }
    ...
    protected function rules()
    {
        return [
            ...
            'package_categories.frequency' => ['nullable', 'string', 'in:'.implode(',', array_keys(packageFrequency()))],
            'package_categories.no_of_visits' => ['nullable', 'integer', 'min:0'],
        ];
    }
    ...
    protected $messages = [
        ...
        'package_categories.frequency.in' => 'The frequency must be one of: daily, weekly, bi_weekly, monthly',
        'package_categories.no_of_visits.integer' => 'The number of visits must be an integer',
        'package_categories.no_of_visits.min' => 'The number of visits must be at least 0',
    ];

    public function save($close = false)
    {
        abort_unless(auth()->user()?->can($this->table_id ? 'package category.edit' : 'package category.create'), 403);
        $this->validate();
        try {
            if (! $this->table_id) {
