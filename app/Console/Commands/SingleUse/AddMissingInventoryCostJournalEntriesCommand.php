...
class AddMissingInventoryCostJournalEntriesCommand extends Command
{
    ...
    public function handle()
    {
        ...
        // Get account IDs
        $accounts = $this->getAccountIds(['Inventory', 'Cost of Goods Sold']);

        if (empty($accounts['Inventory']) || empty($accounts['Cost of Goods Sold'])) {
            $this->error('Required accounts (Inventory, Cost of Goods Sold) not found in database.');
            return Command::FAILURE;
        }

        $cogsAccountId = $accounts['Cost of Goods Sold'];
        ...
        foreach ($sales as $sale) {
            try {
                ...
                $hasCogsEntry = $journal->entries()
                    ->where(function ($query) use ($cogsAccountId) {
                        $query->where('account_id', $cogsAccountId)
                              ->orWhere('counter_account_id', $cogsAccountId);
                    })
                    ->exists();
