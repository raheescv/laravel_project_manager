...
class SaleReturnService implements SaleReturnRepository {
  HttpService get _http => serviceLocator<HttpService>();

  @override
  Future<SaleReturn> createSaleReturn(Map<String, dynamic> payload) async {
    final data = await _http.post(EndPoints.saleReturn, body: payload);
  ...
  Future<SaleReturnsPage> saleReturns({
    ...
    final data = await _http.get(EndPoints.saleReturn, query: {
  ...
  Future<SaleReturn> saleReturnById(String id) async {
    final data = await _http.get(EndPoints.saleReturnById(id));
    return SaleReturn.fromJson(Map<String, dynamic>.from(data));
  }

  @override
  Future<SaleReturn> updateSaleReturn(
      String id, Map<String, dynamic> payload) async {
    final data = await _http.put(EndPoints.saleReturnById(id), body: payload);
    return SaleReturn.fromJson(Map<String, dynamic>.from(data));
  }

  @override
  Future<ReturnableSale> returnableSale(String saleId) async {
    final data = await _http.get(EndPoints.returnableSale(saleId));
