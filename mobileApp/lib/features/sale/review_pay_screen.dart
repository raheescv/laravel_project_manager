...
class _ReviewPayScreenState extends State<ReviewPayScreen> {
  ...
  Widget _whatsappToggle(CartController cart) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: on ? AstraPalette.successTint : p.card,
  ...
  Widget _statusCard(CartController cart) {
    ...
    final (Color tint, Color color, IconData icon, String title, String desc) = settled
        ? (AstraPalette.successTint, AstraPalette.success, Icons.check_circle, 'Ready to Submit', 'Transaction is fully paid and ready to submit')
        : bal > 0
            ? (AstraPalette.warnTint, const Color(0xFF9A6A00), Icons.warning_amber_rounded, 'Partial Payment', 'Transaction has a remaining balance')
