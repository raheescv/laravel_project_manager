...
/// [DaySessionController]; a successful toggle syncs back to the auth user.
...
class _DaySessionScreenState extends State<DaySessionScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      context.read<DaySessionController>().seedFromUser(context.read<AuthController>().user);
      setState(() {});
    });
  }
  ...
  Widget build(BuildContext context) {
    final c = context.watch<DaySessionController>();
  ...
  // ---------------------------------------------------------------- HERO
  Widget _hero(DaySessionController c, ApiUser? user, String branchName) {
  ...
  // ------------------------------------------------------- DATE + TIME CARD
  Widget _dateTimeCard(DaySessionController c) {
  ...
  // ----------------------------------------------------------- TIMELINE
  Widget _timelineCard(DaySessionController c, ApiUser? user) {
    final open = c.isOpen;
    final openedAt = c.session?.openedAt.isNotEmpty == true
        ? c.session!.openedAt
        : (user?.daySessionOpenedAt ?? '');
  ...
  // ------------------------------------------------------------- NOTICE
  Widget _notice(DaySessionController c) {
    final p = context.astra;
    final open = c.isOpen;
  ...
  // -------------------------------------------------------------- DOCK
  Widget _dock(DaySessionController c) {
    final p = context.astra;
    final open = c.isOpen;
  ...
  Future<void> _pickDate(DaySessionController c) async {
  ...
  Future<void> _pickTime(DaySessionController c) async {
    var temp = c.selected;
  ...
  Future<void> _act(DaySessionController c) async {
    if (c.busy) return;
    if (c.isOpen) {
  ...
  Future<bool?> _confirmClose(DaySessionController c) {
    final p = context.astra;
    final t = context.astraTheme;
